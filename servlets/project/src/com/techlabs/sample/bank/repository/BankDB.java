...
public class BankDB {
	...
	public double getCurrentBalance(String name) {
		Connection conn = makeConn.connect();
		String txnQuery = "SELECT BALANCE FROM BANK_MASTER WHERE NAME=?";
		double balance = -1;
		try {
			stmt = conn.prepareStatement(txnQuery);
			stmt.setString(1, name);
	...
	public List<Transaction> getFilteredTransactions(Date sdate,Date edate) {
		
		Connection conn = makeConn.connect();
		String txnQuery = "SELECT * FROM BANK_TRANSACTION WHERE DATE BETWEEN ? and ?";
		transactions = new ArrayList<Transaction>();
		try {
			stmt = conn.prepareStatement(txnQuery);
